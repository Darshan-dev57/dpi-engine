...
```text
								 +-----------------------------+
								 | Start classifyFlow(flow,pkt)|
								 +--------------+--------------+
																|
											flow.sni already set?
												 / yes          \ no
												v                v
										 return      TCP && dstPort==443 ?
																		 / yes       \ no
																		v             v
													try SniExtractor      skip
																		|
														sni found?
														/ yes   \ no
													 v         v
								flow.sni=sni;app=fromSni   continue
																			|
												 TCP && dstPort==80 && sni empty?
															/ yes              \ no
														 v                    v
										try HttpHostExtractor        skip
														 |
													host found?
													/ yes   \ no
												 v         v
						 flow.sni=host;app=fromSni   continue
																		|
												 app UNKNOWN and (src/dst port 53)?
														 / yes                 \ no
														v                       v
											 app = DNS               fallback by port
																									443=>HTTPS
																									80 =>HTTP
```
